    ...
    stage('Creating Deployment') {
      steps {
        ...
        sh  '''#!/bin/bash
                ...
                then
                    kubectl set image deployment/aes-app1 nginx=misbah012/calculator:$BUILD_ID-$BRANCH_NAME -n $BRANCH_NAME
                elif [[ $GIT_BRANCH == "master" ]]
                then
                    kubectl set image deployment/aes-app1 nginx=misbah012/calculator:$BUILD_ID-$BRANCH_NAME -n production
                fi         
            '''
      }
    }
